...
public class ASTGenerator {
    ...
    public List<StmtNode> scanStatements() {
    ...
    private void scanNextToken() {
        while (currentToken.tokenType() != TokenType.TOKEN_EOF) {
            ...
            if (currentToken.tokenType() == TokenType.TOKEN_ERROR) {
                hadError = true;
                continue;
            }
    ...
    private void errorAt(Token identifierToken, String errMsg) {
        ...
        // Print Error
        if (identifierToken.tokenType() != TokenType.TOKEN_ERROR)
            Depends.on(IOUtils.class).err.printf("Error at line %d: %s%n", identifierToken.lineNo(), errMsg);
    }
}
